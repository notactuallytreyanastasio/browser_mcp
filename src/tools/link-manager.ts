...
export class LinkManager {
  constructor(private database: Database, private _server: any) {}

  async queryLinks(args: QueryLinksArgs): Promise<MCPResult> {
    try {
      ...
      const linkList = links.map((link, index) => {
        const score = link.score && link.score > 0 ? `⭐ ${link.score}/5` : '';
        const curated = link.isCurated ? '✅' : '';
        const tags = link.tags.length > 0 ? `🏷️ ${link.tags.join(', ')}` : '';
        const notes = link.notes ? `📝 ${link.notes}` : '';
        
        return `**${index + 1}. ${link.title}** ${curated} ${score}
🔗 ${link.url}
📍 ${link.sourceSite} | ${link.sourcePage || 'Unknown page'}
📅 ${link.extractedAt?.toLocaleDateString()}
${tags}
${notes}`;
      }).join('\n\n');
      ...
      throw new DatabaseError(`Error querying links: ${(error as Error).message}`, error as Error);
    }
  }

  async curateLink(args: CurateLinkArgs): Promise<MCPResult> {
    ...
    try {
      ...
      throw new DatabaseError(`Error curating link: ${(error as Error).message}`, error as Error);
    }
  }

  async getBagOfLinks(count: number = 10, minDaysOld: number = 2, maxDaysOld: number = 90): Promise<MCPResult> {
    try {
...
Selected links span ${new Set(diverseLinks.map(l => l.source_site)).size} different sources.`
      ...
      throw new DatabaseError(`Error generating bag of links: ${(error as Error).message}`, error as Error);
    }
  }

  private ensureSourceDiversity(links: any[], maxCount: number): any[] {
  ...
  private generateBagOfLinksHtml(links: any[]): string {
    const now = new Date();
...
<body>
  ...
  async generateTagCloud(maxTags: number = 50, linksPerTag: number = 5): Promise<MCPResult> {
    try {
      ...
      const tagData = await this.database.executeSql(tagsQuery, [maxTags]) as TagData[];
...
Click any tag to see associated links!`
      ...
      throw new DatabaseError(`Error generating tag cloud: ${(error as Error).message}`, error as Error);
    }
  }

  private generateTagCloudHtml(tagData: TagData[], linksPerTag: number): string {

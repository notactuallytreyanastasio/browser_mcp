...
class BrowserMCPServer {
  ...
  private redditExtractor: RedditExtractor;
      ...
      try {
        switch (name) {
          ...
          case 'get_top_stories':
            ...
            return await this.redditExtractor.getTopStories(args.count as number || 3);
          
          case 'get_top_stories_multi':
            this.redditExtractor.browser = this.browser;
            this.redditExtractor.page = this.page;
            return await this.redditExtractor.getTopStoriesMulti(
              args.sites as string[], 
              args.count as number || 10, 
              args.format as 'markdown' | 'json' || 'markdown', 
              args.save_links !== false
            );
  ...
  private async openBrowser(): Promise<MCPResult> {
    if (this.browser) {
      return {
        content: [{
          type: 'text',
    ...
    try {
      ...
      throw new BrowserError(`Failed to open browser: ${(error as Error).message}`, error as Error);
    }
  }
  ...
  private async navigateTo(url: string): Promise<MCPResult> {
    ...
    try {
      ...
      throw new BrowserError(`Failed to navigate: ${(error as Error).message}`, error as Error);
    }
  }

  private async closeBrowser(): Promise<MCPResult> {
    if (!this.browser) {
      return {
        content: [{
          type: 'text',
    ...
    try {
      ...
      throw new BrowserError(`Failed to close browser: ${(error as Error).message}`, error as Error);
    }
  }

...
export class Database implements DatabaseConnection {
  ...
  async init(customPath: string | null = null): Promise<void> {
  ...
  async getSiteId(domain: string): Promise<number> {
    return new Promise((resolve, reject) => {
      this.db!.get(
        'SELECT id FROM sites WHERE domain = ?',
        [domain],
        (err, row: SiteRecord | undefined) => {
          if (err) {
  ...
  async saveInteraction(domain: string, interaction: Interaction): Promise<number> {
  ...
  async savePattern(domain: string, pattern: Pattern): Promise<number> {
  ...
  async getInteractions(domain: string, limit: number = 50): Promise<InteractionRecord[]> {
    const siteId = await this.getSiteId(domain);
  ...
  async incrementPatternSuccess(patternId: number): Promise<void> {
    return new Promise((resolve, reject) => {
      this.db!.run(
  ...
  async saveLink(linkData: Partial<Link>): Promise<number> {
    const {
      title,
      url,
      sourceSite,
    ...
    return new Promise((resolve, reject) => {
      ...
      this.db!.get(
        ...
        [url],
        (err, existingLink: Pick<LinkRecord, 'id' | 'saved_at'> | undefined) => {
          if (err) {
  ...
  async getLinks(filters: LinkFilters = {}): Promise<Link[]> {
    ...
    return new Promise((resolve, reject) => {
      this.db!.all(sql, params, (err, rows: LinkRecord[]) => {
        if (err) {
  ...
  async updateLink(id: number, updates: Partial<Link>): Promise<number> {
    const allowedFields = [
  ...
  async saveArchive(archiveData: ArchiveData): Promise<number> {
    const {
      linkId = null,
      url,
      title,
        ...
        function(err) {
          ...
          if (resources.length > 0) {
            const resourcePromises = resources.map(resource => {
              return new Promise<void>((resolveResource, rejectResource) => {
                this.db!.run(
  ...
  async getArchives(filters: ArchiveFilters = {}): Promise<ArchiveRecord[]> {
    ...
    return new Promise((resolve, reject) => {
      this.db!.all(query, params, (err, rows: ArchiveRecord[]) => {
        if (err) {
  ...
  async getArchiveById(archiveId: number): Promise<ArchiveRecord | null> {
    return new Promise((resolve, reject) => {
      this.db!.get(
        `SELECT a.*, l.title as link_title, l.url as link_url, l.tags 
         ...
         WHERE a.id = ?`,
        [archiveId],
        (err, row: ArchiveRecord | undefined) => {
          if (err) {
  ...
  async getArchiveResources(archiveId: number): Promise<ArchiveResourceRecord[]> {
    return new Promise((resolve, reject) => {
      this.db!.all(
        `SELECT * FROM archive_resources WHERE archive_id = ? ORDER BY resource_type, original_url`,
        [archiveId],
        (err, rows: ArchiveResourceRecord[]) => {
          if (err) {
  ...
  async getSchema(): Promise<Array<{ name: string; sql: string }>> {
    return new Promise((resolve, reject) => {
      this.db!.all(
        "SELECT name, sql FROM sqlite_master WHERE type='table' ORDER BY name",
        [],
        (err, rows: Array<{ name: string; sql: string }>) => {
          if (err) {
  ...
  async deletePattern(domain: string, patternName: string): Promise<number> {
    return new Promise((resolve, reject) => {
      // First, get the site_id for the domain
      this.db!.get("SELECT id FROM sites WHERE domain = ?", [domain], (err, site: SiteRecord | undefined) => {
        if (err) {
  ...
  private getTableInfo(tableName: string): Promise<Array<{ name: string }>> {
    return new Promise((resolve) => {
      this.db!.all(`PRAGMA table_info(${tableName})`, [], (err, columns: Array<{ name: string }>) => {
        if (err) {
  ...
  private runMigrationsSequentially(migrations: string[], needsBackfill: boolean): Promise<void> {
    return new Promise((resolve, reject) => {
      let completed = 0;

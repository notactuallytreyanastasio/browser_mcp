export class LearningModeService {
    ...
    patterns = new Map();
    constructor(mcpClient, database) {
        this.mcpClient = mcpClient;
        this.database = database;
    }
    async startLearningSession(name, targetUrl, options = {}) {
    ...
    async validatePattern(pattern, testUrl) {
        ...
        try {
          ...
          `);
                    ...
                    errors.push(`Rule '${rule.field}' failed: ${error.message}`);
                }
            }
    ...
    async applyPattern(patternId, url) {
    ...
    findElementByDescription(snapshot, description) {
        const desc = description.toLowerCase();
        // Try exact text match first
        let element = snapshot.find(el => el.text?.toLowerCase().includes(desc) ||
    ...
    findElementsByDescription(snapshot, description) {
        const desc = description.toLowerCase();
    ...
    async extractDataFromElement(element, fieldName) {
        const script = `
    ...
    async generatePatternsFromInteractions(interactions) {
    ...
    async analyzeFieldInteractions(fieldName, interactions) {
        // Find common patterns in selectors
        const selectors = interactions.map(i => i.element?.selector).filter(Boolean);
    ...
    calculatePatternConfidence(pattern) {
        if (pattern.validationResults.length === 0)
            return 0.5;
    ...
    async savePatternsToDatabase(domain, patterns) {
        for (const pattern of patterns) {
            try {

...
The page has been fully archived with all resources for offline viewing.`
            ...
            throw new BrowserError(`Archive failed: ${error.message}`, error);
        }
    }
   ...
   📅 Archived: ${new Date(archive.archived_at).toLocaleDateString()}`;
            ...
            throw new BrowserError(`Failed to list archives: ${error.message}`, error);
        }
    }
...
📸 **Screenshot:** ${archive.screenshot_path ? 'Available' : 'Not captured'}`;
            ...
            throw new BrowserError(`Failed to browse archive: ${error.message}`, error);
        }
    }
    ...
    guessContentType(url) {
        ...
        if (urlLower.includes('.css') || urlLower.includes('text/css'))
            return 'css';
